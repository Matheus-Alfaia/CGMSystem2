...
?>
	...
	<div class="home-section">
	    ...
	    <div class="pesq-btn">
	    	<form method="POST" action="Pesquisar-Produto-Extra.php">
    		<input type="text" class="in" name="Pesquisar" id="Pesquisar" placeholder="PESQUISAR">
    		<input type="submit" class="btn" value="ENVIAR">
			</form>
	    </div>
	    <?php
	    define("TITULO", "PESQUISA");
		include "cabecalho.php";
		include_once "conn/conexao.php";
		include "funcoes.php";
		$pesquisa = $_POST['Pesquisar'];
    	$result_pesq = "SELECT * FROM produto WHERE NomeP LIKE '$pesquisa%'";
    	$resultado_pesquisa = mysqli_query($conexao, $result_pesq);
		while($rows_pesquisa = mysqli_fetch_array($resultado_pesquisa)){
			...
			$n = 1;
			foreach($resultado_pesquisa as $rows_pesquisa){
				echo '<tr>';
				echo '<td>'.$n.'</td>';
				echo '<td>'.$rows_pesquisa["NomeP"].'</td>';
				echo '<td>'.$rows_pesquisa["Descricao"].'</td>';
				echo '</td>';
				echo '<td><a href="eventos-perfil.php?IdProduto='.$rows_pesquisa["IdProduto"].'"><i class="bx bx-file-find"></i></a></td>';
				echo '<td><a href="eventos-editar.php?IdProduto='.$rows_pesquisa["IdProduto"].'"><i class="bx bx-edit-alt"></i></a></td>';
				echo '<td><a href="eventos-excluir.php?IdProduto='.$rows_pesquisa["IdProduto"].'"><i class="bx bx-message-alt-x"></i></td></tr>';
				echo '</tr>';
				$n += 1;
			}
		echo "</tbody>";
