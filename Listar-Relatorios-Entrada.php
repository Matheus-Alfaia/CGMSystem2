	...
	<div class="home-section">
	    <div class="home-content">
	      <i class='bx bx-menu'></i>
			...
			define("TITULO", "LISTAR RELATÓRIOS");
			...
			echo "</thead>";
			echo "<tbody>";
			$relEnt 	= listarRelatorio($conexao);
            // $idP = listarProduto($conexao);
			$n = 1;
			// echo "Total: ".count($eventos);
			foreach($relEnt as $re){

				echo '<tr>';
				echo '<td>'.$n.'</td>';
				$idprod = $re['idProduto'];
                $listP = listarProduto($conexao);
                foreach ($listP as $p) {
                    if($p['IdProduto'] == $idprod){
                        echo '<td>'.$p['NomeP'].'</td>';
                    }
                }
                $idFor = $re['idFornecedor'];
                $fornecedor = listarFornecedor($conexao);
                foreach ($fornecedor as $f) {
                    if($f['IdFornecedor'] == $idFor){
                        echo '<td>'.$f['Nome'].'</td>';
                    }
                }
				echo '<td>'.$re["quantidade"].'</td>';
                echo '<td>'.$re["codigoBarra"].'</td>';
				echo '</tr>';
				$n += 1;
			}
		echo "</tbody>";
